...
/**
 * Tableau d'appareils réutilisable — mêmes colonnes, tri et panneau de
 * détails partout où une liste d'appareils Livebox est affichée : l'onglet
 * "Appareils" (tous les appareils) et l'onglet "Répéteurs" (appareils
 * rattachés à un répéteur donné), pour une présentation cohérente.
 */
...
function DeviceDetailDrawer({ device, onClose, onRenamed }) {
  ...
  const handleSaveType = async (deviceType) => {
    setSavingType(true);
    try {
      await runAction(
        { type: "livebox/topology/type/set", mac: device.mac, device_type: deviceType || null },
        { success: deviceType ? "Type d'appareil forcé enregistré." : "Détection automatique du type restaurée." },
      );
      setEditingType(false);
      onRenamed?.(); // refresh the list, same as after a rename
        ...
        <div className="space-y-4 p-4">
          ...
          <div>
            ...
            <div className="flex justify-between border-b lb-border py-1 text-sm last:border-0">
              <span className="lb-text-muted">Type</span>
              {editingType ? (
                <div className="flex items-center gap-1.5">
                  <select
                    defaultValue={device.type ?? ""}
                    disabled={savingType}
                    onChange={(e) => handleSaveType(e.target.value)}
                    className="lb-input py-0.5 text-xs"
                    autoFocus
                  >
                  ...
                  <button onClick={() => setEditingType(false)} className="rounded-md p-1 hover:bg-[var(--secondary-background-color)]">
                    <X className="size-4 lb-text-muted" />
                  </button>
                </div>
              ) : (
                <span className="flex items-center gap-1.5 font-medium lb-text">
                  {deviceTypeLabel(device.type) ?? "—"}
                  <button onClick={() => setEditingType(true)} className="rounded p-0.5 hover:bg-[var(--secondary-background-color)]" title="Modifier le type">
                    <Pencil className="size-3 lb-text-muted" />
                  </button>
                </span>
              )}
            </div>
...
/**
 * @param {object[]} devices - liste d'appareils au format `livebox/devices`
 *   (déjà filtrée par l'appelant si besoin, ex: appareils d'un répéteur donné)
 */
          ...
          <tbody>
            {sorted.map((d) => (
              <tr key={d.mac} className="border-t lb-border hover:bg-[var(--secondary-background-color)]/50">
                ...
                <td className="py-1.5 pr-3 lb-text-muted">{deviceTypeLabel(d.type) ?? "—"}</td>
